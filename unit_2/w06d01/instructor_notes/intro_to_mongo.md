...
[Our Cheatsheet](https://git.generalassemb.ly/sei-stamford/sei-skynet/blob/master/unit_2/w06d01/homework/Mongo_Hamburgers_Hotels/Mongo_Cheatsheet.md)
...
Let's use two terminal tabs. One to keep bash open and available and one more to run the mongo shell.

<!-- In terminal, type `mongod` -->
 If you've already installed Mongo, go to your terminal and enter `brew services start mongodb`
...
If this does not work, that means that your computer is not connected to Mongo. To fix, run `brew services restart mongodb`
...
```js
db.contacts.find({ state:'California' })
```
...
### Remember to quit out of the Mongo Shell when you are done.

To quit out of the Mongo shell type `CTRL + C`
