...
Now we have our POST request set up correctly

### Middleware

We know how to create functionality for each route. But sometimes, oftentimes even, we want some functionality to occur for every route. We can do this by adding `middleware`. Middleware runs in the middle of a request-response cycle.
...
Express comes with a built in function that lets us handle this problem
...
P.S. This used to be in a separate npm package called `body-parser` but it later became part of express. When you look at examples online you may see people using the body-parser package
