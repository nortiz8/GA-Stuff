...
- Javascript handles this for us with a method called `document.ready` also referred to as `window.onload` (yes these are slightly different, but they are close enough for us) - googling either will help you find the right code to use.
...
- We've been covering a lot of things with a lot of little details. No need to memorize, [just keep this cheatsheet handy](https://git.generalassemb.ly/sei-stamford/sei-skynet/blob/master/unit_1/w03d02/instructor_notes/jQueryCheatsheetTable.md)! or [this one](jQueryCheatsheet1.md) - they have the same information but are formatted differently, use the one that is easier for you to read
