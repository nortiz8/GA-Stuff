...
* The handler function will add a class `pink` that will make the _target_ element pink. To locate the target element, use `$(event.currentTarget)`.

Even though we set the _same_ mouseover listener and handler on each div, we can target each individual div with `$(event.currentTarget)`.
