...
We can also add strings together with something called *template literals*. With template literals, we can avoid using the `+` sign by using backticks. Let's try by modifying the "What a beautiful day" example.
...
What are some of the benefits that we can see from using template literals instead of string concatenation?
...
* Extra
	...
	a. Output a console log "The sum of 5 and 10 is 15" where the values for 5 and 10 are saved to variables, and where 15 comes from those variables being summed.
...
- Write another _while_ loop, but write it from memory. Make the loop count from 0 to 5000 and print the square of each number.
